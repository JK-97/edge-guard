...
//ImagesList is
func (c *DockerObj) ImagesList() (*schema.DockerImageResp, error) {
	...
	//}
	s := make([]*schema.DockerImageResources, 0)
	for _, image := range images {
		//str, _ := json.Marshal(image)
		//fmt.Printf("%s\n", str)
		image_info := &schema.DockerImageResources{Tag: image.RepoTags, ImageID: image.ID, Lable: image.Labels}
		s = append(s, image_info)

	}

	dockerresp := &schema.DockerImageResp{Status: "runing", ProcessPercentage: 0.2, DockerInternalResources: s}
	return dockerresp, err
}

func (c *DockerObj) ContainerList() (*schema.DockerContainerResp, error) {
	containers, err := c.cli.ContainerList(c.ctx, types.ContainerListOptions{})
	if err != nil {
		log.Error(err)
	}

	s := make([]*schema.DockerContainerResources, 0)

	for _, container := range containers {
		contaner_info := &schema.DockerContainerResources{Status: container.Status, ContainerID: container.ID, Image: container.Image, Lable: container.Labels}
		s = append(s, contaner_info)
	}
	dockerresp := &schema.DockerContainerResp{Status: "runing", ProcessPercentage: 0.2, DockerInternalResources: s}
	return dockerresp, err
}
...
func (c *DockerObj) DockerRestore() error {
	...
	log.Info("Reading from ", config.InterSettings.Restore.DockerDesc.Path)
	data, err := ioutil.ReadFile(config.InterSettings.Restore.DockerDesc.Path)
	if err != nil {
		// log.Error(err)
		return err
	}
	...
	for filename, info := range m {
		...
		path := filepath.Join(config.InterSettings.Restore.DockerPkg, filename+".tar")
