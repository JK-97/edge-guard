...
func (p *Process) setLog() {
	...
	capture_bytes := 1000
	if capture_bytes > 0 {
		//log.WithFields(log.Fields{"program": p.GetName()}).Info("capture stdout process communication")
		...
		if capture_bytes > 0 {
			//log.WithFields(log.Fields{"program": p.GetName()}).Info("capture stderr process communication")
...
func (p *Process) monitorProgramIsRunning(endTime time.Time, monitorExited *int32, programExited *int32) {
	...
	if atomic.LoadInt32(programExited) == 0 && p.state == STARTING {
		...
		log.WithFields(log.Fields{"program": p.GetName()}).Info("success to start program at pid: ", p.cmd.Process.Pid)
...
func (p *Process) waitForExit(startSecs int64) {
	err := p.cmd.Wait()
	if err != nil {
		//log.WithFields(log.Fields{"program": p.GetName()}).Warn("fail to wait for program exit")
	} else if p.cmd.ProcessState != nil {
		log.WithFields(log.Fields{"program": p.GetName()}).Warnf("program stopped with status:%v", p.cmd.ProcessState)
	} else {
		log.WithFields(log.Fields{"program": p.GetName()}).Warn("program stopped")
	}
	p.lock.Lock()
	defer p.lock.Unlock()
	p.stopTime = time.Now()
...
func (p *Process) run(finishCb func()) {
	...
	for !p.stopByUser {
		...
		if p.state == RUNNING {
			...
			log.WithFields(log.Fields{"program": p.GetName()}).Warn("program exited")
			break
		} else {
			p.changeStateTo(BACKOFF)
		}
...
func (p *Process) Stop(wait bool) {
	...
	p.stopByUser = true
	p.lock.Unlock()
	log.WithFields(log.Fields{"program": p.GetName()}).Warn("stop the program")
	sigs := strings.Fields("")
	...
	go func() {
		...
		if !stopped {
			log.WithFields(log.Fields{"program": p.GetName()}).Warn("force to kill the program")
