...
func (el *EventListener) waitForReady() error {
	log.Debug("start to check if event listener programmanage is ready")
	for {
...
func (pec *ProcCommEventCapture) findEndStr() int {
	...
	if end_pos == -1 {
		if len(pec.eventBuffer) > pec.captureMaxBytes {
			log.WithFields(log.Fields{"programmanage": pec.procName}).Warn("The capture buffer is overflow, discard the content")
			pec.eventBeginPos = -1
			pec.eventBuffer = ""
		}
	}
