...
func NewConfig(configstr string) *Config {
    return &Config{configstr, make(map[string]*ConfigEntry), NewProcessGroup()}
}
...
func (c *Config) GetConfigFileDir() string {
    //return filepath.Dir(c.configstr)
    //
    return "."
}
